...
const TemplatePage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!selectedIssuerSerial) {
            setMessage({ type: 'error', text: 'Morate izabrati CA sertifikat!' });
            return;
        }
        setLoading(true);
        ...
        <div style={styles.container}>
            ...
            <div style={styles.card}>
                ...
                <form onSubmit={handleSubmit} style={styles.form}>
                    ...
                    {/* ISSUER CARD SELECTOR */}
                    <div>
                        <label style={styles.label}>Izaberi CA Sertifikat (Issuer) *</label>
                        <div ref={scrollRef} style={{ display: 'flex', overflowX: 'auto', gap: '12px', paddingBottom: '8px', paddingTop: '4px' }}>
                            {issuers.length === 0
                    ...
                    {/* OSTALA POLJA - pojavljuju se kad je issuer selektovan */}
                    {selectedIssuerSerial && (
                        <>
                            <div style={styles.grid}>
                                <div>
                                    <label style={styles.label}>CN Regex (validacija)</label>
                                    <input style={styles.input} name="cnRegex" placeholder="npr. .*\.ftn\.com" value={formData.cnRegex} onChange={handleChange} />
                                </div>
                                <div>
                                    <label style={styles.label}>SAN Regex (validacija)</label>
                                    <input style={styles.input} name="sanRegex" placeholder="npr. .*\.ftn\.com" value={formData.sanRegex} onChange={handleChange} />
                                </div>
                                <div>
                                    <label style={styles.label}>Maks. trajanje (dani) *</label>
                                    <input style={styles.input} name="ttlDays" type="number" placeholder="npr. 365" value={formData.ttlDays} onChange={handleChange} required min={1} />
                                </div>
                                <div>
                                    <label style={styles.label}>Extended Key Usage</label>
                                    <input style={styles.input} name="extendedKeyUsage" placeholder="npr. serverAuth,clientAuth" value={formData.extendedKeyUsage} onChange={handleChange} />
                                </div>
                            </div>

                            <label style={{ ...styles.label, marginTop: '10px' }}>Key Usage (podrazumevane vrednosti):</label>
                        ...
                        <tbody>
                            {templates.map(t => (
                                <tr key={t.id}>
                                    ...
                                    <td style={styles.td}>
                                        <button onClick={() => handleDelete(t.id)} style={styles.deleteButton}>Obriši</button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
...
const styles = {
    ...
    button: { padding: '12px', backgroundColor: '#27ae60', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '16px', fontWeight: 'bold', marginTop: '5px' },
    messageBox: { padding: '15px', borderRadius: '4px', marginBottom: '10px' },
